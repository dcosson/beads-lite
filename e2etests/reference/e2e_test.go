...
// TestGoldenReference runs golden file tests that compare beads-lite output
// against expected output generated from the reference beads binary.
// Use "make update-e2e" to regenerate expected files from the reference binary.
// Use "make test-e2e-reference" to run these tests.
func TestGoldenReference(t *testing.T) {
	...
	runGoldenTests(t, runner, cases, *update)
}

// TestGoldenLite runs golden file tests for beads-lite-only features
// (e.g., bd graph, cascade blocking, auto-close). Expected output is
// generated from beads-lite itself, not the reference binary.
// Use "make update-lite-e2e" to regenerate expected files.
func TestGoldenLite(t *testing.T) {
	bdCmd := os.Getenv("BD_CMD")
	if bdCmd == "" {
		t.Skip("BD_CMD environment variable not set")
		return
	...
	runner := &Runner{BdCmd: bdCmd, KillDaemons: *updateLite}
	...
	runGoldenTests(t, runner, cases, *updateLite)
}

// generatorVersion returns a header string identifying the binary that
// generated the golden file. Recorded so you can tell which version
// produced the expected output when investigating mismatches.
func generatorVersion(runner *Runner) string {
...
// runGoldenTests runs a set of golden file test cases, optionally updating
// expected output files when isUpdate is true.
func runGoldenTests(t *testing.T, runner *Runner, cases []TestCase, isUpdate bool) {
	t.Helper()

	var versionHeader string
	if isUpdate {
		versionHeader = generatorVersion(runner)
	}
	...
	for _, tc := range cases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			if err != nil {
				hint := "run with -update to generate"
				if tc.LiteOnly {
					hint = "run with -update-lite to generate"
				}
				t.Fatalf("no expected file %q (%s): %v", expectedFile, hint, err)
			}

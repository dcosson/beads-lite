...
func Burn(ctx context.Context, store *issueservice.IssueStore, molID string) (*BurnResult, error) {
	// 1. Load root issue.
...
func cleanExternalDeps(ctx context.Context, store *issueservice.IssueStore, issue *issuestorage.Issue, burnSet map[string]bool) error {
	for _, dep := range issue.Dependencies {
		if !burnSet[dep.ID] {

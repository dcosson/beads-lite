...
test-e2e: build
	...
	BD_CMD=$(realpath $(BD_LITE_CMD)) BD_ACTOR=testactor go test ./e2etests/... -v -count=1 $(ARGS)
...
update-e2e:
	...
	BD_CMD=$(BD_REF_CMD) BD_ACTOR=testactor go test ./e2etests/reference -update -v -count=1 $(ARGS)

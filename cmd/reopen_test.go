package cmd

import (
	"bytes"
	"context"
	"encoding/json"
	"strings"
	"testing"

	"beads2/filesystem"
	"beads2/storage"
)

func TestReopenCmd_ClosedIssue(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	store := filesystem.New(dir)
	if err := store.Init(ctx); err != nil {
		t.Fatal(err)
	}

	// Create and close an issue
	issue := &storage.Issue{Title: "Test issue"}
	id, err := store.Create(ctx, issue)
	if err != nil {
		t.Fatal(err)
	}
	if err := store.Close(ctx, id); err != nil {
		t.Fatal(err)
	}

	var out, errOut bytes.Buffer
	app := &App{
		Storage: store,
		Out:     &out,
		Err:     &errOut,
	}

	cmd := NewReopenCmd(app)
	cmd.SetArgs([]string{id})
	cmd.SetOut(&out)
	cmd.SetErr(&errOut)

	if err := cmd.Execute(); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}

	if !strings.Contains(out.String(), "reopened "+id) {
		t.Errorf("expected output to contain 'reopened %s', got %q", id, out.String())
	}

	// Verify the issue is now open
	reopened, err := store.Get(ctx, id)
	if err != nil {
		t.Fatal(err)
	}
	if reopened.Status != storage.StatusOpen {
		t.Errorf("expected status %q, got %q", storage.StatusOpen, reopened.Status)
	}
	if reopened.ClosedAt != nil {
		t.Error("expected ClosedAt to be nil")
	}
}

func TestReopenCmd_PrefixMatch(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	store := filesystem.New(dir)
	if err := store.Init(ctx); err != nil {
		t.Fatal(err)
	}

	// Create and close an issue
	issue := &storage.Issue{Title: "Test issue"}
	id, err := store.Create(ctx, issue)
	if err != nil {
		t.Fatal(err)
	}
	if err := store.Close(ctx, id); err != nil {
		t.Fatal(err)
	}

	// Use a prefix of the ID
	prefix := id[:len(id)-1]

	var out, errOut bytes.Buffer
	app := &App{
		Storage: store,
		Out:     &out,
		Err:     &errOut,
	}

	cmd := NewReopenCmd(app)
	cmd.SetArgs([]string{prefix})
	cmd.SetOut(&out)
	cmd.SetErr(&errOut)

	if err := cmd.Execute(); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}

	if !strings.Contains(out.String(), "reopened "+id) {
		t.Errorf("expected output to contain 'reopened %s', got %q", id, out.String())
	}
}

func TestReopenCmd_JSON(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	store := filesystem.New(dir)
	if err := store.Init(ctx); err != nil {
		t.Fatal(err)
	}

	// Create and close an issue
	issue := &storage.Issue{Title: "Test issue"}
	id, err := store.Create(ctx, issue)
	if err != nil {
		t.Fatal(err)
	}
	if err := store.Close(ctx, id); err != nil {
		t.Fatal(err)
	}

	var out, errOut bytes.Buffer
	app := &App{
		Storage: store,
		Out:     &out,
		Err:     &errOut,
		JSON:    true,
	}

	cmd := NewReopenCmd(app)
	cmd.SetArgs([]string{id})
	cmd.SetOut(&out)
	cmd.SetErr(&errOut)

	if err := cmd.Execute(); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}

	var result ReopenResult
	if err := json.Unmarshal(out.Bytes(), &result); err != nil {
		t.Fatalf("failed to parse JSON output: %v", err)
	}

	if result.ID != id {
		t.Errorf("expected ID %q, got %q", id, result.ID)
	}
	if result.Status != "reopened" {
		t.Errorf("expected status 'reopened', got %q", result.Status)
	}
}

func TestReopenCmd_NotFound(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	store := filesystem.New(dir)
	if err := store.Init(ctx); err != nil {
		t.Fatal(err)
	}

	var out, errOut bytes.Buffer
	app := &App{
		Storage: store,
		Out:     &out,
		Err:     &errOut,
	}

	cmd := NewReopenCmd(app)
	cmd.SetArgs([]string{"bd-9999"})
	cmd.SetOut(&out)
	cmd.SetErr(&errOut)

	err := cmd.Execute()
	if err == nil {
		t.Error("expected error for non-existent issue")
	}
}

func TestReopenCmd_NotFoundJSON(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	store := filesystem.New(dir)
	if err := store.Init(ctx); err != nil {
		t.Fatal(err)
	}

	var out, errOut bytes.Buffer
	app := &App{
		Storage: store,
		Out:     &out,
		Err:     &errOut,
		JSON:    true,
	}

	cmd := NewReopenCmd(app)
	cmd.SetArgs([]string{"bd-9999"})
	cmd.SetOut(&out)
	cmd.SetErr(&errOut)

	// In JSON mode, errors are output as JSON
	if err := cmd.Execute(); err != nil {
		t.Fatalf("Execute() error = %v", err)
	}

	var result ReopenResult
	if err := json.Unmarshal(out.Bytes(), &result); err != nil {
		t.Fatalf("failed to parse JSON output: %v", err)
	}

	if result.Status != "error" {
		t.Errorf("expected status 'error', got %q", result.Status)
	}
	if result.Error == "" {
		t.Error("expected Error field to be set")
	}
}
